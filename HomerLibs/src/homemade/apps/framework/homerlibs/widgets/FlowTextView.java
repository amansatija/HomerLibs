package homemade.apps.framework.homerlibs.widgets;
...
public class FlowTextView extends RelativeLayout {
	...
	private static double getPointDistance(float x1, float y1, float x2,
			float y2) {
	...
	public void setTextSize(float textSize) {
		this.mTextsize = Utils.getDipValuefor(getContext(), textSize);
		mTextPaint.setTextSize(mTextsize);
		mLinkPaint.setTextSize(mTextsize);
		invalidate();
	}
		...
		for (Box box : boxes) {
			...
			if (box.topLefty > lineYbottom || box.bottomRighty < lineYtop) {
				...
				for (Box innerBox : boxes) {
					if (innerBox.topLefty > lineYbottom
							|| innerBox.bottomRighty < lineYtop) {
...
//						Log.i("tv", "maxWidth: " + maxWidth);
						...
						if (mIsHtml) {
							...
							if (spans.length > 0) {
								actualWidth = parseSpans(lineObjects, spans,
										charOffsetStart, thisCharOffset,
										xOffset);
							} else {
...
//					Log.i("tv", "charOffsetEnd: " + charOffsetEnd);
					...
					for (HtmlObject thisHtmlObject : lineObjects) {
						...
						if (thisHtmlObject instanceof HtmlLink) {
							...
							addLink(thisLink, yOffset, thisLinkWidth,
									lineHeight);
						}
		...
		if (child.getTag() != null) {
			if (child.getTag().toString().equalsIgnoreCase("hideable")) {
				if (yOffset > pageHeight) {
					if (yOffset < boxes.get(boxes.size() - 1).topLefty
							- getLineHeight()) {
...
//		Log.i("flowText", "onMeasure");
	...
	private float parseSpans(ArrayList<HtmlObject> objects, Object[] spans,
			int lineStart, int lineEnd, float baseXOffset) {
		...
		for (Object span : spans) {
			...
			sorterMap.put(spanStart,
					parseSpan(span, tempString, spanStart, spanEnd));
			// objects.add();
		}
		...
		while (!isArrayFull(charFlags)) {
			while (true) {
				...
				while (true) {
					...
					tempString = extractText(spanStart, spanEnd);
					sorterMap.put(spanStart,
							parseSpan(null, tempString, spanStart, spanEnd));
					break;
	...
	private HtmlObject getStyledObject(StyleSpan span, String content,
			int start, int end, float thisXOffset) {
	...
	private HtmlObject getHtmlObject(String content, int start, int end,
			float thisXOffset) {
		HtmlObject obj = new HtmlObject(content, start, end, thisXOffset,
				mTextPaint);
		return obj;
	}
	...
	private HtmlLink getHtmlLink(URLSpan span, String content, int start,
			int end, float thisXOffset) {
		HtmlLink obj = new HtmlLink(content, start, end, thisXOffset,
				mLinkPaint, span.getURL());
		mLinks.add(obj);
		return obj;
	}

	private void addLink(HtmlLink thisLink, float yOffset, float width,
			float height) {
	...
	public void setText(CharSequence text) {
		...
		if (text instanceof Spannable) {
			...
			Object[] urls = mSpannable.getSpans(0, mSpannable.length(),
					Object.class);
		} else {
			mIsHtml = false;
		}
	...
	public BitmapSpec addImage(Bitmap bitmap, int xOffset, int yOffset,
			int padding) {
